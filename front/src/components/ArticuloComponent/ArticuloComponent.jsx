...
function ArticuloComponent() {
 ...
 const handleSort = (event) => {
  ...
  if (sortBy === "desc") {
    ...
    ordenados = [...vistaData].sort((a, b) => b.anio - a.anio);
  } else if (sortBy === "asc") {
    // Ordenar de menor a mayor
    ordenados = [...vistaData].sort((a, b) => a.anio - b.anio);
  } else {
    // Dejar la lista sin ordenar (tal como viene por defecto)
    ordenados = vistaDataOriginal;
  }
...
//ORDENAR ALFABETICAMENTE POR CATEGORIA
const handleSortCategoria = (event) => {
const sortBy = event.target.value;
let ordenadosCategoria;
if (sortBy === "asc") {
  // Ordenar de A a Z por categoría
  ordenadosCategoria = [...vistaData].sort((a, b) => a.categoria.localeCompare(b.categoria));
} else if (sortBy === "desc") {
  // Ordenar de Z a A por categoría
  ordenadosCategoria = [...vistaData].sort((a, b) => b.categoria.localeCompare(a.categoria));
} else {
  // Dejar la lista sin ordenar (tal como viene por defecto)
  ordenadosCategoria = vistaDataOriginal;
}
setVistaData(ordenadosCategoria);
};
  ...
  return (
    <div className="container mx-0">
      ...
      <div className="d-flex flex-row space-between">
        ...
        <div>
        <select
        className="mx-1 px-3 py-2 border-1 rounded rounded-lg border-secondary bg-secondary" //  border-warning y bg-warning 
         onChange={handleSort}
        >
        ...
        <div>
        <select
       className="mx-1 px-3 py-2 border-1 rounded rounded-lg border-secondary bg-secondary"
       onChange={handleSortCategoria}
      >
        <option defaultValue>Ordenar por Categoria</option>
        <option value="asc">Categoría de A a Z</option>
        <option value="desc">Categoría de Z a A</option>
        </select>
